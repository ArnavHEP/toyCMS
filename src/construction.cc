...
{	
	...
	G4Element *elLu = new G4Element("Lutetium", "Lu", 71, 174.97*g/mole);
	...
	if(useMTD)
	{
		G4double mtdInnerRadius = 1.146*m;
		G4double mtdOuterRadius = 1.15*m;
		G4double mtdHalfLength  = 3.6*m;
		G4double mtdCrystalHalfSizeX = 0.002*m;
		G4double mtdCrystalHalfSizeY = 0.02*m; 
		G4double mtdCrystalHalfSizeZ = 0.02*m;
		G4double nMtdRows   = 180;
		G4double nMtdRings  = 180;

		G4Box* solidMTD = new G4Box("solidMTD", mtdCrystalHalfSizeX, mtdCrystalHalfSizeY, mtdCrystalHalfSizeZ);
		logicMTD = new G4LogicalVolume(solidMTD, LYSO, "logicMTD");

		for(G4int i = 0; i < nMtdRows; i++)
		{
			for(G4int j = 0; j < nMtdRings; j++)
			{
				G4Rotate3D rotZMtd(j*(360./nMtdRings)*deg, G4ThreeVector(0,0,1));
				G4Translate3D transXMtd(G4ThreeVector(mtdCrystalHalfSizeY/tan((360/nMtdRings)/2*deg)*mm + mtdCrystalHalfSizeX*mm, 0*mm, -mtdHalfLength + (2*i*mtdCrystalHalfSizeZ)*mm));
				G4Transform3D transformMtd = (rotZMtd)*(transXMtd);

				G4VPhysicalVolume* physMTD = new G4PVPlacement(transformMtd, logicMTD, "physMTD", logicWorld, false, (i*1000)+j, overlapCheck);
				physMTD->GetLogicalVolume()->SetVisAttributes(G4VisAttributes::GetInvisible());
			}
		}

		//marker just for visualization
		G4double markerInnerR = mtdOuterRadius;
		G4double markerOuterR = mtdOuterRadius + 2*mtdCrystalHalfSizeX;
		G4double markerHalfZ  = mtdHalfLength;
		G4Tubs* solidMTDMarker = new G4Tubs("solidMTDMarker", markerInnerR, markerOuterR, markerHalfZ, 0.*deg, 360.*deg);
		G4LogicalVolume* logicMTDMarker = new G4LogicalVolume(solidMTDMarker, Vacuum, "logicMTDMarker");
		new G4PVPlacement(nullptr, G4ThreeVector(), logicMTDMarker, "physMTDMarker", logicWorld, false, 0, overlapCheck);
		auto markerVis = new G4VisAttributes(G4Colour(0.0, 0.6, 1.0, 0.8));
		markerVis->SetForceSolid(true);
		logicMTDMarker->SetVisAttributes(markerVis);
	}
	...
	if(useHCAL)
	{
		...
		for(G4int i = 0; i < hcalLayers; i++) 
		{
			...
			G4LogicalVolume* logicScintLayer = new G4LogicalVolume(solidScintLayer, Scintillator, G4String("logicScintLayer_") + std::to_string(i));
	...
	if(useMuonSystem)
	{
		...
		for(G4int i = 0; i < nMuonSystem; i++)
		{
			for(G4int j = 0; j < dtLayers; j++)
			{
				...
				G4LogicalVolume* logicGasLayer = new G4LogicalVolume(solidGasLayer, ArCO2, G4String("logicGasLayer_") + std::to_string(i));
...
void MyDetectorConstruction::ConstructSDandField()
{
	...
	//MTD
	auto mtdSD = new MtdSensitiveDetector("/MipTimingDetector");
    sdManager->AddNewDetector(mtdSD);
	if (useMTD) {
		logicMTD->SetSensitiveDetector(mtdSD);
	}
	

    // ECAL
    auto ecalSD = new EcalSensitiveDetector("/EmCalorimeter");
    ...
    // HCAL
    auto hcalSD = new HcalSensitiveDetector("/HCalorimeter");
    sdManager->AddNewDetector(hcalSD);
    for (auto lv : hcalScintillatorLVs) {
        lv->SetSensitiveDetector(hcalSD);
    }

    // Drift Tubes
    auto dtSD = new DriftTubeSensitiveDetector("/DriftTube");
    sdManager->AddNewDetector(dtSD);
    for (auto lv : muonGasLVs) {
        lv->SetSensitiveDetector(dtSD);
    }
}
